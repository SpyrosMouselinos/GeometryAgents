...
class MultiLevel:
    ...
    def start_level(self, level_index=None, logged_level=None):
        ...
        if logged_level is not None:
            ...
            self.cur_env.construction, self.cur_env.construction_objects = self.cur_env.generate_construction(
                self.cur_env, self.cur_env.objs)
    ...
    def get_state(self):
        ...
        if self.tool is None:
            ...
            selected_objs = self.tools[self.tool].get_highlighted(self.cur_env)
    ...
    def action_set_tool(self, tool_index):
        # assert(tool is None)
        assert (0 <= tool_index < len(self.tools)
    ...
    def action_click(self, x, y, auto_proceed=False, change_rem_goals=True):
        ...
        if self.moves == self.max_moves:
            finish = True
        elif self.drop_prob > 0 and self.drop_prob > np.random.random():
            finish = True
