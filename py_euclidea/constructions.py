...
def random_point_in_subplane(separation_line, example_pt):
    rnd_pt = np.random.normal(size=2)
    reflected = reflect_by_line(Point(rnd_pt), separation_line)
    pts = [rnd_pt, reflected.a]
    return pts[np.argmin(np.sum((pts - example_pt.a) ** 2, axis=1))]
...
# @j_macke helper functions
def same_point(coor_1, coor_2, tol=0.001):
    if isinstance(coor_1, Point):
        coor_1 = coor_1.a
    if isinstance(coor_2, Point):
        coor_2 = coor_2.a
    return (np.abs(coor_1 - coor_2) < tol).all()


def is_point_on(p, obj):
    coor = obj.closest_on(p)
    return same_point(p, coor)


def same_line(line_1, line_2, tol=0.001):
    p1 = random_point_on(line_1)
    p2 = random_point_on(line_1)
    return is_point_on(p1, line_2) and is_point_on(p2, line_2)


def is_this_goal_satisfied(goal_ob, goal):
    for g in goal:
        if g.identical_to(goal_ob):
            return True
    return False


def main_circle_sectors(circ):
    return [
        Point(circ.c + [circ.r, 0]),
        Point(circ.c + [0, circ.r]),
        Point(circ.c + [-circ.r, 0]),
        Point(circ.c + [0, -circ.r])
    ]
...
def circumcircle_tool(A, B, C):
    """
    Auxiliary Tool not in the original toolset
    """
...
def intersection_ll(line1, line2, exception_on_fail=True):
    """
    Calculates the intersection between two lines.
    Each Line object contains its normal vector (line.n) and its constant (line.c) and its perpendicular line (line.v)
    """
    matrix = np.stack((line1.n, line2.n))
    b = np.array((line1.c, line2.c))
    if abs(np.linalg.det(matrix)) < epsilon:
...
def intersection_lc(line, circle):
    ...
    Each Line contains its normal vector (line.n) and its constant (line.c) and its perpendicular line (line.v)
    Each Circle contains its center point (circle.c) and its radius (circle.r)
    """
    # shift circle to center
    y = line.c - np.dot(line.n, circle.c)
    x_squared = circle.r ** 2 - y ** 2
...
def intersection_tool(point_set1, point_set2, exception_on_fail=True):
    """
    Calculates the intersection between two objects that are called point_sets.
    Each point_set can be a Line or Circle.
    """
    if isinstance(point_set1, Line) and isinstance(point_set2, Line):
        result = intersection_ll(point_set1, point_set2, exception_on_fail=exception_on_fail),
...
def line_tool(point1, point2):
    """
    Creates a Line object by using two points as input.
    Each Point contains a numpy array of coordinates called (a).
    """
...
def perp_tool(line, point):
    """
    Creates a new Line object perpendicular to a given Line object (line) on a specific point (point).
    """
...
def parallel_tool(line, point):
    """
    Creates a new Line object parallel to a given Line object (line) on a specific point (point).
    """
...
def circle_tool(center, point):
    """
    Creates a new Circle object by using two Point objects as input.
    Each Circle contains its center point (circle.c) and its radius (circle.r)
    """
...
def compass_tool(X, Y, center):
    """
    Creates a new Circle object by using three Point objects (X,Y,center) as input.
    The Circle object has a radius equal to the normalised distance between points X and Y.
    The Circle object uses the input point center as its center.
    """
...
def segment_tool(A, B):
    """
    Creates a new Segment object from two Point objects as inputs.
    Each Segment object contains a numpy array of 2 end_points (segment.end_points) and a float length (segment.length).
    """
...
def ray_tool(A, B):
    """
    Creates a new Ray object from two Point objects as inputs.
    Each Ray object contains a constant c (ray.c) and its normalised vector n (ray.n) its perpendicular line v (ray.v)
    """
...
def perp_bisector_tool(A, B):
    """
    Creates a new Line object from two Point objects as inputs.
    The new Line object is perpendicular to the Line created by points A and B, and is located at their midpoint.
    """
    M = (A.a + B.a) / 2
    n = A.a - B.a
    return Line(n, np.dot(M, n))
...
def angle_bisector_tool(A, B, C):
    """
    Creates a new Line object from three Point objects as inputs.
    The new Line object uses B as its starting point.
    The new Line object splits the angle between the Line formed by AB and Line formed by BC in half.
    """
    v1 = A.a - B.a
    v2 = C.a - B.a
    v1 /= np.linalg.norm(v1)
...
def midpoint_tool(A, B):
    """
    Creates a new Point object from two Point objects as inputs.
    The new Point object is located in the middle between points A and B.
    """
    return Point((A.a + B.a) / 2)


class ConstructionProcess:
    def __init__(self, tool_name, clicks, objs):
        self.tool_name = tool_name
        self.clicks = [f for f in objs]
