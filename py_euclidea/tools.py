...
class MoveTool(Tool):
    ...
    def run(self, env, coor, scale):
        ...
        for step in env.steps:
            ...
            raise Exception("Movable step not found")
...
class StdTool(Tool):
    ...
    def run_known_objects(self, env, arg_objects):
        assert len(arg_objects) == len(self.in_types)
        step = ConstrStep(self.tool_f, tuple(arg_objects), self.get_out_types(env, arg_objects))
        self.args = []
        return env.add_and_run(step)

    def run(self, env, coor, scale, max_dist=20):
        obj_type = self.in_types[len(self.args)]
        dist, obj = env.closest_obj_of_type(coor, obj_type)
        dist *= scale
        if obj is None or dist > max_dist:
            self.args = []
            return False
...
class PointTool(Tool):
    def run(self, env, coor, scale, _):
        step = self.prepare_step(env, coor, scale)
        return env.add_and_run(step)

    def prepare_step(self, env, coor, scale):
        max_dist = 20
        objs = list(filter(
...
key_to_tool = {
    ...
    'b': (StdTool(perp_bisector_tool, (Point, Point), Line), 'Perpendicular_Bisector'),
