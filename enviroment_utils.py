...
class EnvironmentUtils:
    ...
    def find_points(mask,
                    ...
                    manhattan_dist=20,
                    threshold=0.5,
                    opposite_corner_detection=True):
        """
        :param mask:
    ...
    def build_image(geom_primitives, out_size, scale, corners, goal_objects, history, visualisation=False):
        point_index = 0
        all_layer = EnvironmentUtils.objects_to_numpy(geom_primitives, out_size, scale, corners, point_index,
                                                      visualisation=visualisation)
        for prim in geom_primitives:
            if type(prim) is Point:
                point_index += 1
        goal_layer = EnvironmentUtils.objects_to_numpy(goal_objects, out_size, scale, corners, point_index,
                                                       visualisation=visualisation)
    ...
    def action_with_keypoint(prediction_index, tool_index, key_point_names, prediction, dataset):
        main_points, succession = EnvironmentUtils.find_points(prediction['masks'][:, :, prediction_index],
                                                               prediction['rois'][prediction_index],
                                                               len(key_point_names))
    ...
    def action_from_prediction(prediction, dataset, environment_tools, action_type_hint=None,
        ...
        for i in range(len(prediction['class_ids'])):
            ...
            if name == 'Point':
                pts, succession = np.array(
                    EnvironmentUtils.find_points(prediction['masks'][:, :, i], prediction['rois'][i], 1))
                if succession is False:
                    continue
                return env_tool_index, pts, i
            if name == 'Line' or name == "Perpendicular_Bisector":
                pts, succession = np.array(
                    EnvironmentUtils.find_points(prediction['masks'][:, :, i], prediction['rois'][i], 2))
                if succession is False:
                    continue
                return env_tool_index, pts, i
            if name == 'Circle':
                reverse = False
                circle_points, succession = EnvironmentUtils.find_points(prediction['masks'][:, :, i],
                                                                         prediction['rois'][i], 2)

                if succession is False:
                    if len(circle_points) == 1 and use_oposite_corner_detection:
                        other_corner = np.array(circle_points[0]) + \
                                       2 * (EnvironmentUtils.get_midle_of_bbox(prediction['rois'][i]) - np.array(
                            circle_points[0]))
            ...
            if name == "Angle_Bisector":
                move, succession = EnvironmentUtils.action_with_keypoint(i, tool_index,
                                                                         ["Angle_RayPoint", "Angle_VertexPoint",
                                                                          "Angle_RayPoint"], prediction, dataset)
                if succession is False:
                    continue
                return env_tool_index, move, i
            if name == "Perpendicular":
                move, succession = EnvironmentUtils.action_with_keypoint(i, tool_index,
                                                                         ["Perpendicular_ToLine",
                                                                          "Perpendicular_ThroughPoint"], prediction,
                                                                         dataset)
                if succession is False:
                    continue
                return env_tool_index, move, i
            ...
            if name == "Compass":
                move, succession = EnvironmentUtils.action_with_keypoint(i, tool_index,
                                                                         ["CompassRadiusPoint", "CompassRadiusPoint",
                                                                          "CompassCenterPoint"], prediction, dataset)
                if succession is False:
                    continue
                return env_tool_index, move, i
